          ...
          <div className="space-y-4 md:space-y-6">
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 md:gap-4">
              <button 
                ...
                className="p-2 hover:bg-slate-100 rounded-full transition-colors cursor-pointer min-w-10 min-h-10 flex items-center justify-center"
              >
                <span className="material-symbols-outlined">arrow_back</span>
              </button>
              <div className="flex-1">
                <h1 className="text-xl md:text-2xl font-bold text-slate-900">{selectedCollection.name}</h1>
                <p className="text-xs md:text-sm text-slate-500">{collectionWallpapers.length} wallpapers in this collection</p>
              </div>
              <button 
                onClick={handleSync}
                disabled={syncing || collectionWallpapers.length === 0}
                className="w-full sm:w-auto bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg md:rounded-xl text-xs md:text-sm font-bold flex items-center justify-center gap-2 transition-all disabled:opacity-50 cursor-pointer"
              >
